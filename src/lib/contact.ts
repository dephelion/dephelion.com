...
const UFEED_LOCAL_PART = [99, 111, 110, 116, 97, 99, 116];
...
/** uFeed's own inbox, so product mail can be filtered from company mail. */
export function decodeUfeedEmail(): string {
  return `${String.fromCharCode(...UFEED_LOCAL_PART)}@${String.fromCharCode(...DOMAIN)}`;
}
