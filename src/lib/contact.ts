...
const FEEDLENS_LOCAL_PART = [99, 111, 110, 116, 97, 99, 116];
...
/** FeedLens's own inbox, so product mail can be filtered from company mail. */
export function decodeFeedlensEmail(): string {
  return `${String.fromCharCode(...FEEDLENS_LOCAL_PART)}@${String.fromCharCode(...DOMAIN)}`;
}
