...
import { UFEED_NAME, UFEED_PATH } from '@/lib/ufeed';
import { breadcrumbLd, pageMetadata } from '@/lib/seo';
import { SITE_NAME } from '@/lib/site';

const TITLE = `${UFEED_NAME} contact and support`;
const DESCRIPTION =
  'How to report a bug in uFeed, suggest a feature, or tell us about a feed it does not work on.';

export const metadata: Metadata = pageMetadata({
  title: TITLE,
  description: DESCRIPTION,
  path: `${UFEED_PATH}contact/`,
});

const breadcrumbs = breadcrumbLd([
  { name: SITE_NAME, path: '/' },
  { name: UFEED_NAME, path: UFEED_PATH },
  { name: 'Contact and support', path: `${UFEED_PATH}contact/` },
]);

export default function UFeedContactPage() {
  return (
    <DocPage
    ...
    >
      ...
      <ul>
        <li>
          ...
          sites change their markup without warning, and when they do uFeed stops finding posts —
      ...
      <h2>Less useful</h2>
      <p>
        A single post blurred that you wanted, or shown that you did not, is usually uFeed working
      ...
      <p className="doc-action">
        <ContactButton inbox="ufeed" label="Show email" />
      </p>

      <p className="doc-note">
        See also the <Link href="/ufeed-browser-extension/privacy/">privacy policy</Link> and the{' '}
